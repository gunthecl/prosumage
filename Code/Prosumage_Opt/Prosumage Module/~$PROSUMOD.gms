...
G_MARKET_PRO2M(h)               Prosumage: Energy generation sold to market
...
objective..
   ...
   Z =E=
         ...
         + sum( (sto,h) , c_var_sto(sto) * ( STO_IN_PRO(res,sto,h) + STO_IN_M(res,sto,h)
                 + STO_OUT_PRO(sto,h) + STO_OUT_M(sto,h)) )
         + sum(  h , price_buy * (G_MARKET_M2PRO(h) + STO_IN_M(res,sto,h) ))
         - sum(  h , price_market(h) * G_MARKET_PRO2M(h) +  STO_OUT_M(sto,h))
;
...
pro_stolev_PRO(sto,h)$( (ord(h)>1) )..
         STO_L_PRO(sto,h) =E= STO_L_PRO(sto,h-1) + sum( res , STO_IN_PRO(res,sto,h))*eta_sto_in - STO_OUT_PRO(sto,h)*eta_sto_out
;


pro_stolev_M(sto,h)$( (ord(h)>1) )..
         STO_L_M(sto,h) =E= STO_L_M(sto,h-1) + STO_IN_M(sto,h)*eta_sto_in - STO_OUT_M(sto,h)*eta_sto_out
;


* Storage level for first period
pro_stolev_start_PRO(sto,'h1')..
        STO_L_PRO(sto,'h1') =E= 0.5 * sto_ini_last(sto) * N_STO_E(sto) + sum( res , STO_IN_PRO(res,sto,'h1'))*eta_sto_in - STO_OUT_PRO(sto,'h1')*eta_sto_out
;


pro_stolev_start_M(sto,'h1')..
        STO_L_M(sto,'h1') =E= 0.5 * sto_ini_last(sto) * N_STO_E(sto) + STO_IN_M(sto,'h1')*eta_sto_in - STO_OUT_M(sto,'h1')*eta_sto_out
;

** Overall storage level
pro_stolev(sto,h)$( (ord(h)>1) )..
         STO_L_PRO(sto,h) =E=   STO_L_PRO(sto,h) +  + STO_L_M(sto,h)
;
...
display d , N_RES_PRO.l , N_STO_E_PRO.l, N_STO_P_PRO.l, Z.l, G_MARKET_M2PRO.l , G_MARKET_PRO2M.l ,
        STO_L_PRO.l, price_market, hh_energy_balance.m
